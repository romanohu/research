# Yu Wang

Yu Wang は UC Berkeley 関連の機械学習研究者で、協調型MARL論文の著者として確認できる。([source](https://openreview.net/profile?id=~Yu_Wang3), [source](https://arxiv.org/abs/2103.04564))

## 調査
- 主要研究領域: reinforcement learning, multi-agent strategic behavior（公開論文実績） ([source](https://openreview.net/profile?id=~Yu_Wang3))
- 個人ホームページ: 未確認（要調査）
- 所属: UC Berkeley（OpenReview 記載） ([source](https://openreview.net/profile?id=~Yu_Wang3))

## 論文
### 2021
- [Discovering Diverse Multi-Agent Strategic Behavior via Reward Randomization](../../MARL/cmarl_papers.html#discovering-diverse-multi-agent-strategic-behavior-via-reward-randomization)
- [The Surprising Effectiveness of PPO in Cooperative, Multi-Agent Games](../../MARL/cmarl_papers.html#the-surprising-effectiveness-of-ppo-in-cooperative-multi-agent-games)
