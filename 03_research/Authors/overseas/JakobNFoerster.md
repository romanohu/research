# Jakob N. Foerster

Jakob N. Foerster は reinforcement learning and mixture-of-experts settings に関連する研究論文の著者として確認できる研究者。([source](https://arxiv.org/abs/2406.18420))

## 調査
- 主要研究領域: reinforcement learning and mixture-of-experts settings ([source](https://arxiv.org/abs/2406.18420))
- 個人ホームページ: 未確認（要調査）
- 所属: 未確認（要調査）


## 論文

### 2024
- [Mixture of Experts in a Mixture of RL settings](../../MARL/marl_papers.md#mixture-of-experts-in-a-mixture-of-rl-settings)

### 2025
- [OVERCOOKEDV2: RETHINKING OVERCOOKED FOR ZERO-SHOT COORDINATION](../../MARL/cmarl_papers.md#overcookedv2-rethinking-overcooked-for-zero-shot-coordination)
